...
contract StakingDeployBase is Script {
    function deploy(address _owner, uint256 _stakingRewards, uint256 _rewardsDuration, ERC20 _lpToken) public {
        Kerosine kerosine = new Kerosine();
        Staking staking = new Staking(_lpToken, kerosine);

        kerosine.transfer(address(staking), _stakingRewards);

        staking.setRewardsDuration(_rewardsDuration);
        staking.notifyRewardAmount(_stakingRewards);

        kerosine.transfer(_owner, kerosine.totalSupply() - _stakingRewards);
