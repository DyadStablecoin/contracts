...
contract VaultLicenser is Owned(msg.sender) {
    ...
    function add(address _vault, bool _isKeroseneVault) external onlyOwner {
        licenses[_vault] = License(true, _isKeroseneVault);
    }

    function remove(address _vault) external onlyOwner {
        licenses[_vault] = License(false, false);
    }

    function isLicensed(address _vault) external view returns (bool) {
        return licenses[_vault].isLicensed;
    }

    function isKerosene(address _vault) external view returns (bool) {
