...
interface IDNft {
    event MintedNft(uint256 indexed id, address indexed to);
    event MintedInsiderNft(uint256 indexed id, address indexed to);
     ...
     */
    function mintNft(address to) external payable returns (uint256 id);
     ...
     */
    function mintInsiderNft(address to) external returns (uint256 id);
