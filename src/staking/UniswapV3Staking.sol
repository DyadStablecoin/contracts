...
contract UniswapV3Staking is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function stake(uint256 noteId, uint256 tokenId) external {
        ...
        require(tickLower >= minTick && tickUpper <= maxTick, "Position tick range out of bounds");

        positionManager.safeTransferFrom(msg.sender, address(this), tokenId);

        stakes[noteId] =
            StakeInfo({liquidity: liquidity, lastRewardTime: block.timestamp, tokenId: tokenId, isStaked: true});
